...
public class KnifeScript : Photon.MonoBehaviour {
	...
	void doHit(int attackID){
		...
		if (mov.doHit(attackID)){
			mov.getCondition().setHealth(-15f);	
		}
	}
	[RPC]
	public void setup(int parentID){
		...
		started=true;
		transform.localScale= new Vector3(.4f,.6f,.4f);
		transform.Translate(new Vector3(0f,-1 * (transform.localScale.y+parent.transform.localScale.y/3),0f));
	}
